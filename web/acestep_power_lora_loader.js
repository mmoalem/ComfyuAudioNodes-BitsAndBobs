...
const NODE_CLASS = "ACEStep Universal Adapter Loader";
const EXT_NAME = "comfyui_acestep_adapter_loader.power_lora_loader";
const LORA_API = "/acestep_adapter_loader/loras";
const ROW_HEIGHT = 24;
...
function parseRowsFromWidgetValues(widgetsValues) {
  ...
  const parseWithLayout = (perRow, globalsCount) => {
    ...
    if (vals.length >= idx + 13) {
      globals.dora_fp32_normalization = Boolean(vals[idx++]);
      globals.dora_slice_fix = Boolean(vals[idx++]);
      globals.dora_adaln_swap_fix = Boolean(vals[idx++]);
      globals.zimage_lumina2_compat = Boolean(vals[idx++]);
      globals.auto_strength_enabled = Boolean(vals[idx++]);
      globals.auto_strength_device = vals[idx++];
      globals.auto_strength_ratio_floor = Number(vals[idx++]);
      globals.auto_strength_ratio_ceiling = Number(vals[idx++]);
      globals.layer_scale_enabled = Boolean(vals[idx++]);
      globals.self_attn_scale = Number(vals[idx++]);
      globals.cross_attn_scale = Number(vals[idx++]);
      globals.ffn_scale = Number(vals[idx++]);
      globals.other_scale = Number(vals[idx++]);
    }
...
function sanitizeState(st) {
  ...
  const globals = {
    ...
    auto_strength_ratio_ceiling: Number.isFinite(+globalsIn.auto_strength_ratio_ceiling)
      ...
      : 1.50,
    dora_fp32_normalization: globalsIn.dora_fp32_normalization !== undefined ? !!globalsIn.dora_fp32_normalization : true,
    ...
    zimage_lumina2_compat: globalsIn.zimage_lumina2_compat !== undefined ? !!globalsIn.zimage_lumina2_compat : false,
    layer_scale_enabled: globalsIn.layer_scale_enabled !== undefined ? !!globalsIn.layer_scale_enabled : false,
    self_attn_scale: Number.isFinite(+globalsIn.self_attn_scale) ? +globalsIn.self_attn_scale : 1.0,
    cross_attn_scale: Number.isFinite(+globalsIn.cross_attn_scale) ? +globalsIn.cross_attn_scale : 1.0,
    ffn_scale: Number.isFinite(+globalsIn.ffn_scale) ? +globalsIn.ffn_scale : 1.0,
    other_scale: Number.isFinite(+globalsIn.other_scale) ? +globalsIn.other_scale : 1.0,
  };
...
function buildUI(node, state, loraValues) {
  ...
  const isSimple = (typeof node.comfyClass === "string" && node.comfyClass.includes("(Simple)")) || 
                   (typeof node.type === "string" && node.type.includes("(Simple)"));
  ...
  if (!isSimple) {
    const wDoraFp32 = node.addWidget(
      "toggle",
      "dora_fp32_normalization",
      !!node._doraGlobals.dora_fp32_normalization,
      (v) => {
        node._doraGlobals.dora_fp32_normalization = !!v;
        persistNodeState(node);
      }
    );
    wDoraFp32.label = "DoRA fp32 normalization (monkey-patch)";
    ...
    wDoraAdaLNSwap.label = "DoRA adaLN swap_scale_shift fix (Flux only)";
    ...
    wZImageCompat.label = "ZiT/Lumina2 auto-fix (Warning: NOT for ACE-Step)";
  }
  ...
  const wLayerScaleEnabled = node.addWidget(
    "toggle",
    "layer_scale_enabled",
    !!node._doraGlobals.layer_scale_enabled,
    (v) => {
      node._doraGlobals.layer_scale_enabled = !!v;
      persistNodeState(node);
    }
  );
  wLayerScaleEnabled.label = "Enable Layer Category Scaling";

  const wSelfAttn = node.addWidget(
    "number",
    "self_attn_scale",
    Number.isFinite(+node._doraGlobals.self_attn_scale) ? +node._doraGlobals.self_attn_scale : 1.0,
    (v) => {
      node._doraGlobals.self_attn_scale = Math.max(0, +v || 0);
      persistNodeState(node);
    },
    { min: 0.0, max: 3.0, step: 0.05 }
  );
  wSelfAttn.label = "self_attn_scale";

  const wCrossAttn = node.addWidget(
    "number",
    "cross_attn_scale",
    Number.isFinite(+node._doraGlobals.cross_attn_scale) ? +node._doraGlobals.cross_attn_scale : 1.0,
    (v) => {
      node._doraGlobals.cross_attn_scale = Math.max(0, +v || 0);
      persistNodeState(node);
    },
    { min: 0.0, max: 3.0, step: 0.05 }
  );
  wCrossAttn.label = "cross_attn_scale";

  const wFfn = node.addWidget(
    "number",
    "ffn_scale",
    Number.isFinite(+node._doraGlobals.ffn_scale) ? +node._doraGlobals.ffn_scale : 1.0,
    (v) => {
      node._doraGlobals.ffn_scale = Math.max(0, +v || 0);
      persistNodeState(node);
    },
    { min: 0.0, max: 3.0, step: 0.05 }
  );
  wFfn.label = "ffn_scale";

  const wOther = node.addWidget(
    "number",
    "other_scale",
    Number.isFinite(+node._doraGlobals.other_scale) ? +node._doraGlobals.other_scale : 1.0,
    (v) => {
      node._doraGlobals.other_scale = Math.max(0, +v || 0);
      persistNodeState(node);
    },
    { min: 0.0, max: 3.0, step: 0.05 }
  );
  wOther.label = "other_scale";
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    const isTarget =
      nodeName.includes("ACEStep Universal Adapter Loader") ||
      displayName.includes("ACEStep Universal Adapter Loader") ||
      comfyClass.includes("ACEStep Universal Adapter Loader");
